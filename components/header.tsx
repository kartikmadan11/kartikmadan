  ...
  return (
    <header className="z-[999] relative">
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:top-[1.7rem] sm:h-[initial] sm:py-0">
        <ul className="flex w-[26rem] flex-wrap items-center justify-center gap-y-1 text-[0.85rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5 dark:text-slate-400">
          {links.map((link) => (
            <motion.li
              ...
              <Link
                ...
                onClick={() => {
                  if (link.hash.startsWith("/")) {
                    // For route links like /blog, don't set active section
                    return;
                  }
